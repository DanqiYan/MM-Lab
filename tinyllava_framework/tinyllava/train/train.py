...
#     debugpy.wait_for_client()  # 直到客户端（VS Code）连接才继续执行
...
from tinyllava.train.trainer_callbacks import SaveCallback, WandbLogCallback
...
# init wandb 
import wandb
wandb.init(project="Compression", entity="Compression_MLLM")
...
def train():
    ...
    try: 
        print(f"Before loading, model is on device: {next(model.parameters()).device}")
    except:
        pass
    ...
    try: 
        print(f"After loading, model is on device: {next(model.parameters()).device}")
    except:
        pass

    # model = model.add_reft_adaptor(training_arguments) if training_arguments.loreft_config_path else model
    ...
    if training_arguments.reft_pos_configs is not None:
        data_arguments.reft_pos_configs = training_arguments.reft_pos_configs
    ...
    # 现在你可以安全地将 `trainer` 传递给回调
    save_callback = SaveCallback(trainer, training_recipe)
    wandb_log_callback = WandbLogCallback(model)

    # 将回调添加到 trainer 中
    trainer.add_callback(save_callback)
    trainer.add_callback(wandb_log_callback)
